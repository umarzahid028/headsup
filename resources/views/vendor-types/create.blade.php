        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('vendor-types.store') }}">
                        @csrf
                        ...
                        <div class="mb-4">
                            <div class="flex items-center">
                                <input id="is_on_site" name="is_on_site" type="checkbox" value="1" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" {{ old('is_on_site') ? 'checked' : '' }}>
                                <label for="is_on_site" class="ml-2 block text-sm text-gray-900">Is On-Site Vendor</label>
                            </div>
                            <div class="flex items-center mt-2">
                                <input id="has_system_access" name="has_system_access" type="checkbox" value="1" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" {{ old('has_system_access') ? 'checked' : '' }}>
                                <label for="has_system_access" class="ml-2 block text-sm text-gray-900">Has System Access</label>
                            </div>
                            <div class="flex items-center mt-2">
                                <input id="is_active" name="is_active" type="checkbox" value="1" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" {{ old('is_active', true) ? 'checked' : '' }}>
