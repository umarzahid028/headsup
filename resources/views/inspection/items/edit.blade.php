    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Edit Inspection Item') }}: {{ $item->name }}
            </h2>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('inspection.items.update', $item) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="inspection_stage_id" name="inspection_stage_id" 
                                    ...
                                    @foreach($stages as $stage)
                                        @if(is_object($stage))
                                            <option value="{{ $stage->id }}" {{ old('inspection_stage_id', $item->inspection_stage_id) == $stage->id ? 'selected' : '' }}>
                                                {{ $stage->name }}
                                            </option>
                                        ...
                                        @else
                                            <option value="{{ $stage }}" {{ old('inspection_stage_id', $item->inspection_stage_id) == $stage ? 'selected' : '' }}>
                                                {{ $stage }}
                                            </option>
                                        @endif
                                    @endforeach
                            ...
                            <div>
                                ...
                                <x-text-input id="order" class="block mt-1 w-full" type="number" name="order" 
                                    :value="old('order', $item->order)" min="1" />
                                <p class="mt-1 text-xs text-gray-500">
                                    Items with lower numbers appear first in checklists
                                </p>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="description" name="description" rows="3" 
                                    class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $item->description) }}</textarea>
                            ...
                            <div>
                                <div class="flex items-center">
                                    <input id="vendor_required" name="vendor_required" type="checkbox" value="1" 
                                        {{ old('vendor_required', $item->vendor_required) ? 'checked' : '' }}
                            ...
                            <div>
                                <div class="flex items-center">
                                    <input id="cost_tracking" name="cost_tracking" type="checkbox" value="1" 
                                        {{ old('cost_tracking', $item->cost_tracking) ? 'checked' : '' }}
                                        class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
